...
const App = () => {
  ...
  // States for nodes, edges, and other properties
  ...
  // Update node data when nodeName or selectedElements changes
  useEffect(() => {
  ...
  // Handle node click to select and update node
  ...
  const onSave = useCallback(() => {
    if (reactFlowInstance) {
      ...
      if (nodes.length > 1 && (emptyTargetHandles > 1 || isNodeUnconnected())) {
        alert('Error: More than one node has an empty target.');
      } else {
